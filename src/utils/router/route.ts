...
>(routeTemplate: RouteTemplate) {
  type RouteBuilder = {
    pattern: RegExp;
  } & (keyof RouteParameters extends never
    ? () => string
  ...
  const parsedFiltered = parsed.filter(({ value }) => value);

  const pattern =
    parsedFiltered.length > 0
      ? new RegExp(
          parsedFiltered
            .map(({ type, value }) => {
              if (type === "param")
  ...
  const builder: RouteBuilder = (...params: any[]) => {
...
export interface RouteBuilderAbstract {
  pattern: RegExp;
